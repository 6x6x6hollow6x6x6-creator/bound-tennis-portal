...
- **シングルスは、出場したカテゴリより対象年齢が上のカテゴリにも算入する**（逆は不可）。
  フリーだけの特別扱いではない。人数の都合でシニアの選手がミドルに出ることがあり、
  その成績がシニアに反映されないと実態と合わない
- **ダブルスとミックスダブルスは跨がない**（`settings.pairEvents` / `isPairEvent`、2026-08-19 決定）。
  ペアのカテゴリは若いほうの相方に合わせて決まるので、下位カテゴリに出ていることが
  本人の実力や意思を表すとは限らない。第44回のフリー男子ダブルスでベスト8以上に残った
  ミドル世代は2名だけで、**どちらも相方はフリーにしか出ていない選手だった**。
  若い相方と組んで取った成績を、45歳以上のペアだけで争うミドルの実力とみなすことはできないし、
  その区分に出続けている選手の上に蓋をしてしまう（実際に堀江和喜がミドルダブルス5位に載っていた）
- **生年は「不明」を許す。** ドロー表には氏名と都道府県しか載っていないので、
  取り込んだ選手の生年は分からない。既定値を入れると（以前は1990年）、
  シニアの選手が若者扱いになって資格の検査に全部弾かれる。
  生年不明のときは **出場したカテゴリから年齢の下限を推定する**（`minAgeOf` / `P_MIN_AGE`）。
  シニアに出ていれば60歳以上と分かるので、その選手がミドルに出た成績もシニアに算入できる。
  年齢は `effAge()`（集計用）と `ageText()`（表示用）を使う。`ageAt()` は不明のとき null を返す
- **出場資格は `catAgeIssue()` で検査する。** カテゴリは下位への一方通行なので、
  62歳はミドルに出られるが30歳はシニアに出られない。年度内に達する年齢との差を
  1歳ぶん猶予して、明らかな取り違えだけを弾く。
  **PDF取り込みの名寄せ間違いは、たいていここに出る**
...
非公式の試作サイトなので、**協会が作ったものと受け取られる表示を出さない**。

- ヘッダー上段は「非公式・検討用の試作サイト」。協会名を出さない
- ロゴは協会の正規マークに寄せない（灰色の中立的な記号にしてある）
- フッターに協会名義の著作権表記を置かない
- 注意バーで「協会の決定事項ではありません」と明示する
- `<meta name="robots" content="noindex, nofollow, noarchive">` と `robots.txt` で検索避け

大会データの中の「主催：公益財団法人 日本バウンドテニス協会」は**事実なので残してよい**。
サイトの発行者を名乗る表示だけを避ける。

サイト文言は `index.html` の `DEFAULT_DATA` と `tools/build-data.mjs` の両方にある。
**片方だけ直すと、JSONを読み込み直したときに元に戻る。**
